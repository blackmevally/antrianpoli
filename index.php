...
@media(max-width:1200px){
  ...
  .side-poli{grid-template-rows:repeat(5,minmax(0,1fr))}
  .side-poli .poli-side-card:nth-child(n+6){display:none}
}
...
/* =============================================================
   ...
   6-10 -> kanan
   11+  -> bawah
   Card bawah TIDAK mengulang kiri/kanan.
   ============================================================= */

function poliUniqueKey(r){
  // Gunakan kode poli + kode dokter sebagai identitas mapping.
  // Satu pasangan poli-dokter hanya boleh muncul sekali.
  return String(r.kd_poli || "") + "|" + String(r.kd_dokter || "");
}

function uniquePoli(data){
  if(!Array.isArray(data)) return [];

  const seen = new Set();

  return data.filter(r => {
    const key = poliUniqueKey(r);
...
function makePoliCard(r, className="poli-side-card"){
  ...
  if(/belum mulai/i.test(nm_pasien)){
    ...
    warna = "linear-gradient(145deg,#b71c1c,#d32f2f)";
  }

  if(className === "poli-card"){
    return `
      <div class="poli-card"
           data-poli-key="${poliUniqueKey(r)}"
...
function renderAllPoli(data){

  // ===========================================================
  // DEDUPLICATE SEKALI DI AWAL
  // ===========================================================
  const d = uniquePoli(data || []);
  ...
  const bawah = d.slice(10);

  // ===========================================================
  // KIRI
  // ===========================================================
  $("#poli-kiri").html(
    kiri.map(r => makePoliCard(r, "poli-side-card")).join("")
  );

  if(!kiri.length){
    $("#poli-kiri").html(
      `<div class="poli-side-card"><h5>Belum ada poli</h5></div>`
    );
  }
  ...
  // ===========================================================
  $("#poli-kanan").html(
    kanan.map(r => makePoliCard(r, "poli-side-card")).join("")
  );
  ...
  // BAWAH
  //
  // HANYA berisi d[10] dst.
  // Jadi card kiri/kanan TIDAK MUNGKIN diulang.
  //
  // Jika jumlah mapping <= 10:
  // bawah = kosong.
  // ===========================================================
  if(bawah.length === 0){
    $("#datapoli").empty().hide();
  ...
  $("#datapoli")
    .html(
      bawah.map(r => makePoliCard(r, "poli-card")).join("")
    )
    .css("display","grid")
    .show();
}
...
function updateDataPoli(){
  $.getJSON(
    "app/antrian.php?p=poli&_=" + Date.now(),
    function(d){
      renderAllPoli(d || []);
    }
  ).fail(function(){
    $("#poli-kiri").html(`
      <div class="poli-side-card"
           style="background:rgba(80,80,80,.9);">
